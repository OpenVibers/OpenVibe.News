...
// ═══════════════════════════════════════════════════════════════
// openvibe.news — OAuth2 CLIENT session layer (sign-in via OpenVibe.Network SSO)
//
// The same session layer OpenVibe.Community uses (server/auth/routes.js there, MIT), with
// News' client id: registered in the Network as OAuth client `news`, redirect
// https://openvibe.news/auth/callback.
//
...
//
// Cookies (host-only for openvibe.news):
        ...
        for (const base of [config.networkInternalUrl, config.networkUrl]) {
            ...
            try {
                ...
                if (jwks.public_key) {
                    ...
                    console.log(`[News] Network public key loaded from ${base} (${jwks.algorithm || 'RS256'})`);
                    return client.publicKey;
                }
            } catch (err) {
                console.warn(`[News auth] JWKS fetch failed from ${base}: ${err.message}`);
            }
        }
        return null;
    }
...
function createAuthRoutes(config, auth) {
    ...
    router.get('/callback', async (req, res) => {
        ...
        try {
            ...
            console.error('[News auth] Code exchange failed:', err.message);
    ...
    router.post('/fedcm', fedcmBody, fedcmBodyError, async (req, res) => {
        ...
        try {
            ...
            console.error('[News auth] FedCM exchange failed:', err.message);
    ...
    router.post('/refresh', async (req, res) => {
        ...
        try {
            ...
            console.error('[News auth] Refresh failed:', err.message);
